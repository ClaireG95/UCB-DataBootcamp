...
app.config["MONGO_URI"] = "mongodb://localhost:27017/mars_app" # tells Python that our app will connect to Mongo using a URI
mongo = PyMongo(app) # is the URI to connect our app to Mongo. This URI is saying that the app can reach Mongo through our localhost server, using port 27017, using a database named ”mars_app”.

# set up 2 Flask routes: 
# one for the main HTML page everyone will view when visiting the web app, 
# and one to scrape new data using the code we’ve written.
